...
function _(t, e) {
  try {
    ...
    }).formatToParts(0).find((r) => r.type === "currency")?.value ?? t;
  } catch {
    return t;
  }
}
...
${s}
    ...
    if (e?.css && e.css.trim() !== "" && r.push(e.css), this.hostCss && this.hostCss.trim() !== "" && r.push(this.hostCss), r.length > 0) {
      ...
      s.setAttribute("data-qaid-q-theme", ""), s.textContent = r.join(`
...
`), this.shadowRoot.appendChild(s);
  ...
  // ------------------------------------------------------------------
  renderStep() {
    const e = this.suppressStepFocus;
    if (this.suppressStepFocus = !1, !this.questionnaire || !this.bodyEl || !this.footerEl) return;
    const i = this.visibleQuestions.length;
    if (i === 0) {
      this.submit();
      return;
    }
...
function ye() {
  ...
  try {
    ...
    return i.cssSelector && !i.css && (i.css = T(i.cssSelector), delete i.cssSelector), i;
  } catch {
    return null;
  }
}
